...
export default function SettingsPage() {
  ...
  const currentNamingPattern =
    settings.find((s: Setting) => s.key === 'silver_naming_pattern')?.value ||
    'datetime';
  const customNamingPattern =
    settings.find((s: Setting) => s.key === 'custom_naming_pattern')?.value || '';
  ...
  const [originalCustomPattern, setOriginalCustomPattern] =
    useState(customNamingPattern);
  ...
  const handleSaveNamingSettings = async () => {
    try {
      ...
      const existingSetting = settings.find(
        (s: Setting) => s.key === 'silver_naming_pattern',
      );
      ...
      if (selectedPattern === 'custom' && customPattern.trim()) {
        const existingCustom = settings.find(
          (s: Setting) => s.key === 'custom_naming_pattern',
        );
        if (existingCustom) {
          await updateSettingMutation.mutateAsync({
  ...
  const hasUnsavedChanges = () => {
    ...
    if (selectedPattern === 'custom' && customPattern !== originalCustomPattern)
      return true;
    return false;
  };
  ...
  const getPreviewFilename = () => {
    const pattern =
      selectedPattern === 'custom'
        ? customPattern
        : namingPatterns.find((p: NamingPattern) => p.id === selectedPattern)
        ...
        <header className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Configure Pictallion to match your workflow preferences
            </p>
          </div>
        </header>
            ...
            <Card>
              <CardHeader>
                ...
                <p className="text-sm text-muted-foreground">
                  Configure how files are named when processed from Bronze to Silver
                  tier. AI descriptions will be generated automatically when available.
                </p>
              </CardHeader>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <code className="text-sm bg-card px-2 py-1 rounded">
                          {
                            namingPatterns.find(
                              (p: NamingPattern) => p.id === selectedPattern,
                            )?.pattern
                          }
                        </code>
                      </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        Use placeholders like {'{year}'}, {'{month}'}, {'{day}'},{' '}
                        {'{hour}'}, {'{minute}'}, {'{second}'}, {'{camera}'},{' '}
                        {'{aiDescription}'}, {'{originalFilename}'}
                      </p>
                    </div>
                  </div>
                )}
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="pl-6 space-y-1 text-muted-foreground">
                        <div>
                          <code>{'{camera}'}</code> - CanonEOSR5
                        </div>
                        <div>
                          <code>{'{lens}'}</code> - 24-70mmf28
                        </div>
                        <div>
                          <code>{'{aiDescription}'}</code> - SunsetBeach
                        </div>
                        <div>
                          <code>{'{originalFilename}'}</code> - IMG_2024
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-800 dark:text-blue-200">
                      When processing photos from Bronze to Silver tier, Pictallion will
                      automatically generate 2-3 word AI descriptions in PascalCase
                      format (e.g., "SunsetBeach", "FamilyDinner"). These descriptions
                      are used in the filename patterns marked with {'{aiDescription}'}.
                    </p>
                  </div>
                </div>
              </CardContent>
                  ...
                  <>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {aiConfig.availableProviders.openai ? (
                        ...
                        >
                          <XCircle className="h-3 w-3 mr-1" />
                          {aiConfig.config.openai.hasApiKey
                            ? 'Key Invalid'
                            : 'No API Key'}
                        </Badge>
                      )}
                ...
                <Tabs defaultValue="general" className="w-full">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(value: any) =>
                          setAiFormData((prev) => ({ ...prev, provider: value }))
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
                  ...
                  <TabsContent value="openai" className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        value={aiFormData.openaiModel}
                        onValueChange={(value) =>
                          setAiFormData((prev) => ({ ...prev, openaiModel: value }))
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
