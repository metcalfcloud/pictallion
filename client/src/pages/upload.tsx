...
export default function Upload() {
  ...
  // Convert upload files to SimpleUpload format for conflict resolution
  const conflictFiles = uploadFiles.filter(f => f.status === 'conflict').map(f => ({
    id: f.id,
    file: f.file,
    status: f.status,
    progress: f.progress,
  ...
  return (
    <>
      ...
      {/* Conflict Resolution Modal - Only show when we have conflicts */}
      {conflictFiles.length > 0 && (
        <SimpleUpload 
          open={showConflictModal} 
          onOpenChange={setShowConflictModal}
          preloadedFiles={conflictFiles}
          onConflictResolved={() => {
            // Update main upload files when conflicts are resolved
            setUploadFiles(current => 
              current.map(file => 
