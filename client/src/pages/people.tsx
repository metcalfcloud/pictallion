...
export default function PeoplePage() {
  const [viewMode, setViewMode] = useState<
    'people' | 'faces' | 'suggestions' | 'ignored'
  >('people');
  const [selectedPerson, setSelectedPerson] = useState<string | null>(null);
  const [selectedFaces, setSelectedFaces] = useState<string[]>([]);
  const [searchQuery, setSearchQuery] = useState('');
  const [assignFacesSearchQuery, setAssignFacesSearchQuery] = useState('');
  const [faceFilter, setFaceFilter] = useState<'all' | 'unassigned' | 'assigned'>(
    'unassigned',
  );
  ...
  const updatePersonMutation = useMutation({
    mutationFn: async ({
      ...
      id: string;
      data: { name: string; notes?: string; birthdate?: string };
    }) => {
  ...
  const handleAssignFaces = useCallback(
    (personId: string) => {
      if (selectedFaces.length > 0) {
  ...
  return (
    <div className="flex-1 overflow-auto bg-background dark:bg-gray-900">
      <div className="p-6">
        ...
        {(viewMode === 'people' || viewMode === 'faces') && (
          <div className="flex items-center space-x-4 mb-6">
            <div className="flex-1 relative">
              ...
              <Input
                placeholder={
                  viewMode === 'people' ? 'Search people...' : 'Search faces...'
                }
                value={searchQuery}
            ...
            {viewMode === 'faces' && (
              <div className="flex items-center space-x-2">
                ...
                <Select
                  value={faceFilter}
                  onValueChange={(value: 'all' | 'unassigned' | 'assigned') =>
                    setFaceFilter(value)
                  }
                >
        ...
        {viewMode === 'suggestions' && <FaceSuggestions isOpen={true} />}
            ...
            {ignoredFacesLoading ? (
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                {Array.from({ length: 12 }).map((_, i) => (
                  <div
                    key={i}
                    className="w-32 h-32 bg-muted rounded animate-pulse"
                  ></div>
                ))}
              </div>
                ...
                {ignoredFaces.map((face) => (
                  <div key={face.id} className="relative group">
                    <div className="w-32 h-32 bg-muted rounded overflow-hidden">
                      ...
                      <div
                        className={`w-full h-full flex items-center justify-center ${face.faceCropUrl ? 'hidden' : ''}`}
                      >
              ...
              <Card>
                <CardContent className="p-12 text-center">
                  ...
                  <p className="text-muted-foreground">
                    You haven't ignored any faces yet. Use the "Ignore Face" option in
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-center space-x-3 mb-3">
                        <div className="w-12 h-12 bg-muted rounded-full flex items-center justify-center overflow-hidden">
                          ...
                          <User
                            className={`w-6 h-6 text-muted-foreground ${person.coverPhoto ? 'hidden' : ''}`}
                          />
                        </div>
                        <div>
                ...
                <CardContent className="p-12 text-center">
                  ...
                  <p className="text-muted-foreground mb-4">
                    {searchQuery
                      ...
                      : 'Start by adding people to organize your photos'}
                  </p>
                  <Button
                    onClick={() => {
                      setNewPersonName('');
                              ...
                              {group.faces.length > 1 && (
                                <Button
                                  ...
                                  onClick={() => {
                                    ...
                                    setSelectedFaces((prev) => {
                                      const allSelected = groupFaceIds.every((id) =>
                                        prev.includes(id),
                                      );
                                      if (allSelected) {
                                        // Deselect all if all are selected
                                        return prev.filter(
                                          (id) => !groupFaceIds.includes(id),
                                        );
                                      } else {
                                        // Select all that aren't selected
                                        const newSelections = groupFaceIds.filter(
                                          (id) => !prev.includes(id),
                                        );
                              ...
                              <div className="flex items-center justify-between mb-2">
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    const groupFaceIds = group.faces.map((f) => f.id);
                                    setSelectedFaces((prev) =>
                                      prev.filter((id) => !groupFaceIds.includes(id)),
                                    );
                                  }}
                              ...
                              <div className="space-y-3">
                                ...
                                <div className="relative">
                                  ...
                                  <Input
                                    ...
                                    onChange={(e) =>
                                      setAssignFacesSearchQuery(e.target.value)
                                    }
                                    className="pl-10"
                                  />
                                </div>
                                    ...
                                    {(() => {
                                      const filteredPeople = people.filter((person) =>
                                        person.name
                                          .toLowerCase()
                                          .includes(
                                            assignFacesSearchQuery.toLowerCase(),
                                          ),
                                      );
                                      ...
                                      if (filteredPeople.length > 0) {
                                        return filteredPeople.map((person) => (
                                          <button
                                            ...
                                            onClick={() => {
                                              const selectedInGroup = group.faces
                                                .filter((f) =>
                                                  selectedFaces.includes(f.id),
                                                )
                                                .map((f) => f.id);
        ...
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            <DialogTitle>
              ...
              {people.find((p) => p.id === selectedPerson)?.name || 'Person'}
            </DialogTitle>
            <DialogDescription>
          ...
          <div className="grid grid-cols-6 gap-4 max-h-96 overflow-y-auto">
            {selectedPerson &&
              (() => {
                const personFaces = faces.filter(
                  (face) => face.personId === selectedPerson,
                );
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Select a person to assign the {selectedFaces.length} selected face
              {selectedFaces.length !== 1 ? 's' : ''} to.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="relative">
            ...
            <div className="grid grid-cols-2 gap-3 max-h-64 overflow-y-auto">
              {people
                .filter((person) =>
                  person.name
                    .toLowerCase()
                    .includes(assignFacesSearchQuery.toLowerCase()),
                )
                .map((person) => (
                  <Button
                    key={person.id}
