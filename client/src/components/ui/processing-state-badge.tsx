...
export function FaceDetectionBadge({
  status,
  faceCount = 0,
  size = 'sm',
}: FaceDetectionBadgeProps) {
  const getBadgeConfig = () => {
    switch (status) {
      case 'success':
        return {
...
export function getFaceDetectionStatus(photo: any): {
  status: 'success' | 'failed' | 'none' | 'processing';
  faceCount: number;
} {
  // Check if face detection was attempted
  const faceDetectionAttempted =
    photo.metadata?.faceDetection?.attempted || photo.detectedFaces !== undefined;
  ...
  const faceDetectionFailed =
    photo.metadata?.faceDetection?.failed || photo.metadata?.faceDetection?.error;
...
export function getProcessingState(
  photo: any,
): 'unprocessed' | 'processed' | 'promoted' | 'rejected' {
