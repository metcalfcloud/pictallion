...
(async () => {
  ...
  await runTest("Upload supports preview and drag-and-drop", async () => {
    ...
    if (!(await page.$('[data-testid="upload-preview"]')))
  ...
  await runTest("Metadata Viewer shows metadata", async () => {
    ...
    if (!(await page.$('[data-testid="metadata-table"]')))
  ...
  await runTest("Face Detection works", async () => {
    ...
    if (!(await page.$('[data-testid="face-detection-result"]')))
      throw new Error("No face detection result");
  });
  ...
  await runTest("Bulk actions are available", async () => {
    ...
    if (!(await page.$('[data-testid="bulk-action-btn"]')))
      throw new Error("No bulk action button");
  });
  ...
  await runTest("Onboarding/help modal appears", async () => {
    ...
    if (!(await page.$('[data-testid="onboarding-modal"]')))
      throw new Error("No onboarding modal");
  });

  await runTest("User settings/profile accessible", async () => {
    await page.click('[data-testid="user-settings-nav"]');
    await page.waitForSelector('[data-testid="user-profile"]');
    if (!(await page.$('[data-testid="user-profile"]')))
  ...
  await runTest("Responsive layout (mobile)", async () => {
    ...
    if (!(await page.$('[data-testid="mobile-nav"]')))
      throw new Error("No mobile nav");
  });
