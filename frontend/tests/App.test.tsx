...
describe('App Component with Tauri Integration', () => {
  ...
  it('should detect Tauri environment correctly', () => {
    ...
    expect(screen.getByText(/pictallion/i)).toBeInTheDocument();
  });
  ...
  it('should handle Tauri API calls from Upload component', async () => {
    ...
    // Navigate to Upload tab if it exists
    const uploadTab = screen.queryByText(/upload/i);
    if (uploadTab) {
      ...
      if (fileInput) {
        const file = new File(['test content'], 'test-app.jpg', { type: 'image/jpeg' });
  ...
  it('should handle multiple component interactions with Tauri APIs', async () => {
    ...
    const galleryTab = screen.queryByText(/gallery/i);
    if (galleryTab) {
      ...
      await waitFor(() => {
        ...
        const hasPhotoCall = callHistory.some(call => call.command === 'list_photos');
        if (hasPhotoCall) {
    ...
    if (peopleTab) {
      ...
      await waitFor(() => {
        ...
        const hasPeopleCall = callHistory.some(call => call.command === 'list_people');
        if (hasPeopleCall) {
  ...
  it('should maintain app state during Tauri API operations', async () => {
    ...
    const initialText = screen.getByText(/pictallion/i);
