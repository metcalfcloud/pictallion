...
class SearchFilters:
    ...
    perceptual_hash_similarity: Optional[Dict[str, Union[str, float]]] = None  # {'hash': str, 'threshold': float}


@dataclass
class SortOptions:
    """Sort options for search results"""
    field: str = 'createdAt'  # 'createdAt', 'rating', 'fileSize', 'confidence', 'eventName'
...
class AdvancedSearchService:
            ...
            async with get_session() as session:
                ...
                if filters.date_range:
                    if filters.date_range.get('start'):
                        conditions.append(FileVersion.created_at >= filters.date_range['start'])
                    if filters.date_range.get('end'):
                        conditions.append(FileVersion.created_at <= filters.date_range['end'])
                ...
                if filters.query:
                    ...
                    # Search in file path
                    text_conditions.append(func.lower(FileVersion.file_path).contains(query_lower))
                    
                    # Search in location
                    text_conditions.append(func.lower(FileVersion.location).contains(query_lower))
                    
                    # Search in event name
                    text_conditions.append(func.lower(FileVersion.event_name).contains(query_lower))
                    
                    # Search in AI short description (JSON field)
                    text_conditions.append(
                        func.lower(FileVersion.ai_short_description).contains(query_lower)
                    )
                ...
                if filters.location:
                    conditions.append(
                        func.lower(FileVersion.location).contains(filters.location.lower())
                    )
                
                if filters.event_name:
                    conditions.append(
                        func.lower(FileVersion.event_name).contains(filters.event_name.lower())
                    )
                ...
                # Get total count
                count_query = select(func.count(FileVersion.id)).select_from(query.subquery())
                ...
                for photo in photos:
                    ...
                    photo_dict = {
                        ...
                        'mediaAsset': {
                            'originalFilename': asset.original_filename if asset else 'Unknown'
                        },
        ...
        try:
            async with get_session() as session:
                ...
                source_result = await session.execute(
                    select(FileVersion.perceptual_hash)
                    .where(FileVersion.id == photo_id)
                )
                ...
                for photo, original_filename in all_photos:
                    if photo.perceptual_hash:
                        similarity = self._calculate_hash_similarity(source_hash, photo.perceptual_hash)
                        if similarity >= threshold:
                            similar_photos.append(SimilarPhoto(
                                id=photo.id,
                ...
                for collection in smart_collections:
                    ...
                    try:
                        ...
                        await session.execute(
                            text("DELETE FROM collection_photos WHERE collection_id = :collection_id")
                        ...
                        print(f"Updated smart collection '{collection.name}' with {len(matching_photos)} photos")
                        
                    except Exception as e:
                        print(f"Failed to update smart collection {collection.name}: {e}")
        ...
        try:
            ...
            for photo in all_photos:
                ...
                if photo.rating:
                    ...
                    facets['ratings'][rating_str] = facets['ratings'].get(rating_str, 0) + 1
                
                # Count event types
                if photo.event_type:
                    facets['eventTypes'][photo.event_type] = facets['eventTypes'].get(photo.event_type, 0) + 1
                
                # Count MIME types
                if photo.mime_type:
                    facets['mimeTypes'][photo.mime_type] = facets['mimeTypes'].get(photo.mime_type, 0) + 1
                ...
                if photo.metadata and isinstance(photo.metadata, dict):
                    ...
                    if exif and isinstance(exif, dict):
                        ...
                        if camera:
                            facets['cameras'][camera] = facets['cameras'].get(camera, 0) + 1
                
                # Count keywords
                if photo.keywords and isinstance(photo.keywords, list):
                    for keyword in photo.keywords:
                        facets['keywords'][keyword] = facets['keywords'].get(keyword, 0) + 1
    ...
    async def _find_photos_matching_rules(self, session, smart_rules: Dict[str, Any]) -> List[str]:
        """
...
async def find_similar_photos(photo_id: str, threshold: float = 85.0, limit: int = 20) -> List[SimilarPhoto]:
    """Find visually similar photos using perceptual hash"""
