...
class ReverseGeocodingService:
    ...
    def __init__(self):
        ...
        self.request_delay = (
            1.0  # 1 second delay between requests per Nominatim usage policy
        )
    ...
    async def reverse_geocode(
        ...
        try:
            ...
            headers = {"User-Agent": "Pictallion Photo Management System/1.0"}
            ...
            async with httpx.AsyncClient() as client:
                ...
                if not response.is_success:
                    print(
                        f"Reverse geocoding failed: {response.status_code} {response.reason_phrase}"
                    )
                    return None
    ...
    async def geocode_if_needed(
        self, latitude: float, longitude: float, existing_location: Optional[str] = None
    ) -> Optional[str]:
        """
...
def are_coordinates_similar(
    ...
    return ReverseGeocodingService.are_coordinates_similar(
        lat1, lon1, lat2, lon2, tolerance_meters
    )
