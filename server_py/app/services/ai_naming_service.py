...
class AINamingService:
    ...
    def apply_naming_pattern(self, pattern: str, context: NamingContext) -> str:
        ...
        if context.exif_metadata:
            ...
            for date_field in ["dateTimeOriginal", "createDate", "dateTime"]:
                if (
                    date_field in context.exif_metadata
                    and context.exif_metadata[date_field]
                ):
                    try:
        ...
        if context.ai_metadata and context.ai_metadata.get("shortDescription"):
    ...
    async def generate_silver_filename(
        self, context: NamingContext, naming_pattern: str
    ) -> str:
        """
        ...
        extension = (
            context.original_filename.split(".")[-1]
            if "." in context.original_filename
            else "jpg"
        )
