...
from app.models.schemas import ExifMetadata, CombinedMetadata
        ...
        try:
            if self._is_image_file(original_filename):
                ...
                if exif_data.date_time_original:
                    ...
                    if photo_date:
                        logger.info(f"Using photo date from EXIF DateTimeOriginal: {photo_date}")
                
                if not photo_date and exif_data.create_date:
                    photo_date = self._parse_exif_date(exif_data.create_date)
                    if photo_date:
                        logger.info(f"Using photo date from EXIF CreateDate: {photo_date}")
                        
                if not photo_date and exif_data.date_time:
                    photo_date = self._parse_exif_date(exif_data.date_time)
                    if photo_date:
                        logger.info(f"Using photo date from EXIF DateTime: {photo_date}")
        ...
        def _sync_extract_exif():
            ...
            try:
                ...
                if hasattr(img, '_getexif') and img._getexif():
                    ...
                    metadata.date_time_original = self._safe_get_string(exif.get('DateTimeOriginal'))
                    metadata.create_date = self._safe_get_string(exif.get('DateTime'))
                    metadata.date_time = metadata.date_time_original or metadata.create_date
                    ...
                    if 'FocalLength' in exif:
                        metadata.focal_length = self._format_focal_length(exif['FocalLength'])
