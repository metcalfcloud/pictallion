...
class ThumbnailService:
    ...
    async def generate_thumbnail(
        self, original_path: str, options: ThumbnailOptions
    ) -> str:
        """
    ...
    async def _create_thumbnail(
        self, original_path: str, output_path: Path, options: ThumbnailOptions
    ) -> None:
    ...
    def get_quality_preset(self, preset: str) -> ThumbnailOptions:
        ...
        """
        presets = {
            "low": ThumbnailOptions(size=150, quality=60, width=150, height=150),
            "medium": ThumbnailOptions(size=300, quality=80, width=300, height=300),
            "high": ThumbnailOptions(size=600, quality=95, width=600, height=600),
            "thumbnail": ThumbnailOptions(size=200, quality=75, width=200, height=200),
        }

        return presets.get(preset, presets["medium"])

    async def get_thumbnail(self, original_path: str, options: ThumbnailOptions) -> str:
        """
        ...
        thumbnail_options = ThumbnailOptions(
            size=size, quality=options.quality, format="jpeg"
        )

        return await self.generate_thumbnail(original_path, thumbnail_options)

    async def batch_generate_thumbnails(
        self, file_paths: list[str], options: ThumbnailOptions, max_concurrent: int = 5
    ) -> Dict[str, str]:
        """
        ...
        logger.info(
            f"Generated {len(thumbnail_map)} thumbnails out of {len(file_paths)} requested"
        )
        return thumbnail_map

    async def cleanup_orphaned_thumbnails(self, valid_original_paths: set[str]) -> int:
        """
