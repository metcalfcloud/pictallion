...
class DuplicateDetectionService:
    ...
    async def extract_file_metadata(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"Extracted metadata for duplicate detection - {file_path}: {metadata}")
            return metadata
                            ...
                            if hasattr(img, '_getexif') and img._getexif() is not None:
                                ...
                                for tag_id, value in exif.items():
                                    ...
                                    if tag == 'Make':
                                        ...
                                        exif_dict['shutter'] = f"1/{round(1/value)}s" if value > 0 else str(value)
                                ...
                                if 'make' in exif_dict and 'model' in exif_dict:
                                    exif_dict['camera'] = f"{exif_dict['make']} {exif_dict['model']}"
    ...
    async def check_for_duplicates(
        ...
        """
        print(f"=== DUPLICATE CHECK START: {original_filename} with hash {file_hash} ===")
        ...
        try:
            async with get_session() as session:
                ...
                if exact_duplicate:
                    ...
                    asset_result = await session.execute(
                        select(MediaAsset).where(MediaAsset.id == exact_duplicate.media_asset_id)
                    )
                    asset = asset_result.scalar_one_or_none()
                    
                    if asset:
                        print(f"=== FOUND EXACT MD5 DUPLICATE ===")
                        print(f"Existing file: {asset.original_filename} (hash: {exact_duplicate.file_hash})")
                ...
                if is_image:
                    new_perceptual_hash = await self.generate_perceptual_hash(temp_file_path)
                    # Extract metadata for new file for burst detection
                    new_file_metadata = await self._extract_direct_metadata(temp_file_path)
                    ...
                    if new_perceptual_hash:
                        ...
                        print(f"Found {len(all_photos)} existing photos to compare against")
                        ...
                        for photo in all_photos:
                            ...
                            if photo.file_hash == file_hash:
                                print(f"Skipping exact duplicate comparison: {photo.file_hash} === {file_hash}")
                                continue
                            
                            # Only compare with images
                            if not photo.mime_type or not photo.mime_type.startswith('image/'):
                                continue
                            ...
                            if not existing_perceptual_hash:
                                full_path = os.path.join(self.settings.data_directory, photo.file_path)
                                try:
                                    if await aiofiles.os.path.exists(full_path):
                                        existing_perceptual_hash = await self.generate_perceptual_hash(full_path)
                                        
                                        # Store the perceptual hash for future use
                                        if existing_perceptual_hash:
                                            photo.perceptual_hash = existing_perceptual_hash
                                            session.add(photo)
                            ...
                            if existing_perceptual_hash:
                                ...
                                if existing_perceptual_hash in conflicted_hashes:
                                    print(f"Skipping duplicate conflict for hash {existing_perceptual_hash} - already conflicted")
                                    continue
                                
                                # Require 100% perceptual match to avoid false positives with burst photos
                                print(f"Similarity check: {similarity}% between {original_filename} and {photo_asset.original_filename}")
                                ...
                                if similarity >= 100.0:
                                    ...
                                    if is_burst_photo:
                                        print(f"Detected burst photo sequence: {original_filename} vs {photo_asset.original_filename} ({similarity}%) - allowing as separate photos")
                                    ...
                                    print(f"Perfect perceptual match non-burst conflict: {original_filename} vs {photo_asset.original_filename} ({similarity}%)")
                                    ...
                                    conflict = DuplicateConflict(
                                        ...
                                        suggested_action=self._suggest_action(reasoning, original_filename, photo_asset.original_filename),
                                        reasoning=reasoning
                                    )
            ...
            print(f"=== DUPLICATE CHECK COMPLETE: {original_filename} - {len(conflicts)} conflicts found ===")
            return conflicts
    ...
    def _analyze_metadata_differences(
        ...
        new_has_editing_keywords = any(keyword in new_filename.lower() for keyword in editing_keywords)
        existing_has_editing_keywords = any(keyword in existing_filename.lower() for keyword in editing_keywords)
        
        if new_has_editing_keywords and not existing_has_editing_keywords:
            reasons.append('New file appears to be edited version (filename contains editing keywords)')
        elif not new_has_editing_keywords and existing_has_editing_keywords:
            reasons.append('Existing file appears to be edited version (filename contains editing keywords)')
        
        # Check EXIF modification dates
        if (existing_metadata and 
            existing_metadata.get('exif', {}).get('modifyDate') and 
            existing_metadata.get('exif', {}).get('dateTimeOriginal')):
            
            try:
                modify_date = datetime.fromisoformat(existing_metadata['exif']['modifyDate'])
                original_date = datetime.fromisoformat(existing_metadata['exif']['dateTimeOriginal'])
                if modify_date > original_date:
                    reasons.append('Existing file shows signs of modification (modify date after original date)')
        ...
        if not reasons:
            reasons.append('Files appear visually identical with different metadata - manual review recommended')
        
        return '; '.join(reasons)
    
    def _suggest_action(self, reasoning: str, new_filename: str, existing_filename: str) -> str:
        """
    ...
    async def _is_burst_photo(
        ...
        try:
            ...
            def get_photo_time(metadata: Optional[Dict[str, Any]], filename: str, fallback_timestamp: Optional[str] = None) -> float:
                ...
                if fallback_timestamp:
                    try:
                        return datetime.fromisoformat(fallback_timestamp).timestamp() * 1000
                    except (ValueError, TypeError):
                        pass
                
                return datetime.now().timestamp() * 1000
            
            time1 = get_photo_time(existing_metadata, existing_filename, existing_created_at)
            ...
            name1 = existing_filename.lower().replace('.jpg', '').replace('.jpeg', '').replace('.png', '').replace('.tiff', '')
            name2 = new_filename.lower().replace('.jpg', '').replace('.jpeg', '').replace('.png', '').replace('.tiff', '')
            ...
            if (existing_metadata and existing_metadata.get('exif') and 
                new_metadata and new_metadata.get('exif')):
                
                exif1 = existing_metadata['exif']
                exif2 = new_metadata['exif']
                
                # Same camera and lens = higher burst likelihood
                same_camera = (exif1.get('make') == exif2.get('make') and 
                              exif1.get('model') == exif2.get('model'))
                same_lens = (exif1.get('lens') == exif2.get('lens') or 
                            exif1.get('focalLength') == exif2.get('focalLength'))
                
                # Similar camera settings (burst photos typically have consistent settings)
                similar_settings = (
                    exif1.get('iso') == exif2.get('iso') and
                    exif1.get('aperture') == exif2.get('aperture') and
                    exif1.get('whiteBalance') == exif2.get('whiteBalance')
                )
                
                # If we have timing proximity + same camera + similar settings = likely burst
                if time_diff <= 10000 and same_camera and (same_lens or similar_settings):  # 10 seconds
                    return True
    ...
    async def process_duplicate_resolution(
        ...
        try:
            if action == 'keep_existing':
                ...
                async with get_session() as session:
                    result = await session.execute(
                        select(FileVersion).where(FileVersion.id == conflict.existing_photo['id'])
                    )
                    ...
                    return {
                        ...
                        "assetId": existing_file.media_asset_id if existing_file else None
                    }
    ...
    async def _replace_existing_file(self, conflict: DuplicateConflict) -> Dict[str, Any]:
        ...
        # Remove the old file
        old_file_path = os.path.join(self.settings.data_directory, conflict.existing_photo['filePath'])
        try:
        ...
        silver_path = await file_manager_service.process_to_silver(
            conflict.new_file['tempPath'],
            conflict.new_file['originalFilename']
        )
        ...
        async with get_session() as session:
            ...
            if mime_type.startswith('image/'):
                try:
                    ...
                    ai_short_description = ai_metadata.get('shortDescription') if ai_metadata else None
                except Exception as ai_error:
                    print(f"AI analysis failed for {conflict.new_file['originalFilename']}: {ai_error}")
...
async def check_for_duplicates(temp_file_path: str, original_filename: str, file_hash: str) -> List[DuplicateConflict]:
    """Check for duplicates during upload process"""
    return await duplicate_detection_service.check_for_duplicates(temp_file_path, original_filename, file_hash)


async def process_duplicate_resolution(conflict_id: str, action: str, conflict: DuplicateConflict) -> Dict[str, Any]:
    """Process user's resolution of duplicate conflicts"""
    return await duplicate_detection_service.process_duplicate_resolution(conflict_id, action, conflict)
