...
US_HOLIDAYS: List[HolidayDefinition] = [
    ...
    HolidayDefinition(
        "Martin Luther King Jr. Day", 1, 15, "US"
    ),  # Third Monday approximation
...
class EventDetectionService:
    ...
    async def detect_events(self, photo_date: datetime) -> List[EventMatch]:
        ...
        try:
            ...
            self.logger.info(
                f"Detected {len(matches)} events for date {photo_date.date()}"
            )
            return matches
    ...
    def _detect_holidays(
        self, photo_date: datetime, enabled_countries: List[str]
    ) -> List[EventMatch]:
        """
        ...
        for holiday in all_holidays:
            ...
            clean_name = (
                holiday.name.lower().replace(" ", "_").replace("'", "").replace(".", "")
            )
            # Exact date match
            ...
            async with get_async_session() as session:
                ...
                for event in events:
                    ...
                    if event.is_recurring and event.recurring_type == "yearly":
                        ...
                        check_date = date(
                            photo_date.year, event_date.month, event_date.day
                        )
                    else:
    ...
    def calculate_age_in_photo(
        self, birthdate: date, photo_date: datetime
    ) -> Optional[int]:
        """
        ...
        try:
            ...
            photo_date_only = (
                photo_date.date() if isinstance(photo_date, datetime) else photo_date
            )
            ...
            if photo_date_only.month < birthdate.month or (
                photo_date_only.month == birthdate.month
                and photo_date_only.day < birthdate.day
            ):
                age -= 1
