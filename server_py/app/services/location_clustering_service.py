...
class LocationClusteringService:
    ...
    def calculate_distance(
        ...
        a = math.sin(d_lat / 2) * math.sin(d_lat / 2) + math.cos(
            self._to_radians(lat1)
        ) * math.cos(self._to_radians(lat2)) * math.sin(d_lon / 2) * math.sin(d_lon / 2)
    ...
    def extract_coordinates(
        ...
        for photo in photos:
            ...
            if (
                gps_coords
                and gps_coords.get("latitude", 0) != 0
                and gps_coords.get("longitude", 0) != 0
            ):
                ...
                if (
                    exif_metadata.get("gpsLatitude") is not None
                    and exif_metadata.get("gpsLongitude") is not None
                ):
        ...
        self.logger.info(
            f"Extracted coordinates from {len(photos_with_location)} of {len(photos)} photos"
        )
        return photos_with_location

    def cluster_photos(
        self, photos: List[PhotoWithLocation], max_distance_meters: float = 100
        ...
        for photo in photos:
            ...
            for other_photo in photos:
                ...
                distance = self.calculate_distance(
                    photo.latitude,
                    photo.longitude,
                    other_photo.latitude,
                    other_photo.longitude,
                )
    ...
    def find_hotspots(
        self, photos: List[Dict[str, Any]], min_photo_count: int = 10
    ) -> List[LocationHotspot]:
        """
    ...
    def _generate_suggested_name(self, cluster: LocationCluster) -> Optional[str]:
        ...
        if photos:
            place_names = []
            for photo in photos:
                if photo.metadata and photo.metadata.get("ai", {}).get("placeName"):
        ...
        event_types = []
        for photo in photos:
            if photo.metadata and photo.metadata.get("ai", {}).get("detectedEvents"):
    ...
    def calculate_location_stats(
        self, photos: List[Dict[str, Any]], existing_locations: List[Any]
    ) -> LocationStats:
        """
        ...
        return LocationStats(
            ...
            coverage_percentage=(
                ((photos_with_coords / total_photos) * 100) if total_photos > 0 else 0
            ),
            average_photos_per_location=(
                (photos_with_coords / location_count) if location_count > 0 else 0
            ),
        )
    ...
    def get_location_bounds(
        self, photos: List[PhotoWithLocation]
