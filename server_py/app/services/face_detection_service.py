...
face_recognition library, dlib, and advanced heuristic methods.
Converts TypeScript FaceDetectionService to Python with equivalent functionality.
"""
...
from app.core.crud import face as face_crud, person as person_crud
...
class FaceDetectionService:
    ...
    Uses face_recognition library with multiple detection methods including
    neural networks and advanced heuristic analysis for comprehensive coverage.
    """
    ...
    async def initialize_face_recognition(self) -> bool:
        """Initialize face recognition library."""
        if self.face_recognition_initialized:
            return True
        
        try:
            logger.info("Initializing face_recognition library...")
            
            # Test basic functionality
            def _test_face_recognition():
            ...
            await loop.run_in_executor(None, _test_face_recognition)
    ...
    async def detect_faces(self, image_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            faces = await self._detect_faces_with_face_recognition(image_path)
            
            if faces:
                logger.info(f"face_recognition detected {len(faces)} faces successfully")
                metadata["face_detection"]["method"] = "face_recognition"
            else:
                # Fall back to advanced heuristic analysis
                logger.info("face_recognition found no faces, using advanced analysis fallback")
                faces = await self._detect_faces_with_advanced_analysis(image_path)
                metadata["face_detection"]["method"] = "advanced_analysis" if faces else "none"
    ...
    async def _detect_faces_with_face_recognition(self, image_path: str) -> List[DetectedFace]:
        """Detect faces using face_recognition library."""
        try:
            await self.initialize_face_recognition()
            
            if not self.face_recognition_initialized:
                logger.warning("face_recognition not available, will use fallback detection")
                return []
            ...
            def _sync_detect_faces():
                """Synchronous face detection for thread pool."""
                # Load image
                image = face_recognition.load_image_from_file(str(full_image_path))
                ...
                for i, (location, encoding) in enumerate(zip(face_locations, face_encodings)):
                    ...
                    face = DetectedFace(
                        id=f"face_rec_{int(datetime.now().timestamp() * 1000)}_{i}_{hash(str(location)) % 10000:04d}",
                        bounding_box=(x, y, width, height),
                        confidence=85,  # face_recognition is generally reliable
            ...
            logger.info(f"face_recognition detected {len(faces)} faces")
            return faces
    ...
    async def _detect_faces_with_advanced_analysis(self, image_path: str) -> List[DetectedFace]:
        """Detect faces using advanced analysis methods."""
        try:
            full_image_path = self.data_dir / image_path
            ...
            logger.info(f"Advanced analysis of {width}x{height} image for face detection")
            
            # Step 1: Find skin-tone regions
            skin_regions = await self._find_skin_tone_regions(str(full_image_path), width, height)
            
            # Step 2: Apply composition rules
            composition_regions = await self._find_face_regions_using_composition(width, height)
            
            # Step 3: Combine approaches
            candidate_regions = self._combine_face_regions(skin_regions, composition_regions, width, height)
            
            faces = []
            for i, region in enumerate(candidate_regions):
                # Generate embedding for the region
                embedding = await self._generate_face_embedding(image_path, region.bounding_box)
    ...
    async def _find_skin_tone_regions(self, image_path: str, width: int, height: int) -> List[FaceRegion]:
        ...
        try:
            def _sync_find_skin_regions():
                ...
                skin_regions = self._find_skin_color_clusters(rgb_image, target_width, target_height)
                ...
                for region in skin_regions:
                    ...
                    face_area = scaled_region.bounding_box[2] * scaled_region.bounding_box[3]
    ...
    def _find_skin_color_clusters(self, image: np.ndarray, width: int, height: int) -> List[FaceRegion]:
        """Find connected regions of skin-colored pixels."""
        regions = []
        
        # Skin tone color ranges for different ethnicities
        skin_ranges = [
            {"r_min": 170, "r_max": 255, "g_min": 140, "g_max": 220, "b_min": 120, "b_max": 200},  # Light
            {"r_min": 140, "r_max": 200, "g_min": 100, "g_max": 170, "b_min": 80, "b_max": 140},   # Medium
            {"r_min": 100, "r_max": 160, "g_min": 70, "g_max": 130, "b_min": 50, "b_max": 100},    # Dark
        ]
        ...
        for y in range(height):
            for x in range(width):
                ...
                for skin_range in skin_ranges:
                    if (skin_range["r_min"] <= r <= skin_range["r_max"] and
                        skin_range["g_min"] <= g <= skin_range["g_max"] and
                        skin_range["b_min"] <= b <= skin_range["b_max"]):
                        skin_mask[y, x] = True
                        break
        ...
        for y in range(height):
            for x in range(width):
                if skin_mask[y, x] and not visited[y, x]:
                    region_info = self._flood_fill_region(skin_mask, visited, x, y, width, height)
    ...
    def _flood_fill_region(self, mask: np.ndarray, visited: np.ndarray, start_x: int, start_y: int, width: int, height: int) -> Dict[str, int]:
    ...
    async def _find_face_regions_using_composition(self, width: int, height: int) -> List[FaceRegion]:
        ...
        if width > height * 1.3:
            ...
            for pos in positions:
                if (search_area["start_x"] <= pos["x"] <= search_area["end_x"] and
                    search_area["start_y"] <= pos["y"] <= search_area["end_y"]):
            ...
            logger.debug("Detected portrait orientation - estimating single/couple portrait")
    ...
    def _combine_face_regions(self, skin_regions: List[FaceRegion], composition_regions: List[FaceRegion], width: int, height: int) -> List[FaceRegion]:
        ...
        for comp_region in composition_regions:
            ...
            for existing in combined_regions:
                overlap = self._calculate_region_overlap(comp_region.bounding_box, existing.bounding_box)
    ...
    def _calculate_region_overlap(self, region1: Tuple[int, int, int, int], region2: Tuple[int, int, int, int]) -> float:
    ...
    async def _generate_face_embedding(self, image_path: str, bounding_box: Tuple[int, int, int, int]) -> Optional[List[float]]:
        ...
        try:
            ...
            return embedding or self._generate_fallback_embedding(image_path, bounding_box)
            
        except Exception as e:
            logger.error(f"Failed to generate face embedding: {e}")
            return self._generate_fallback_embedding(image_path, bounding_box)
    
    def _generate_fallback_embedding(self, image_path: str, bounding_box: Tuple[int, int, int, int]) -> List[float]:
    ...
    async def find_similar_faces(self, face_embedding: List[float], threshold: float = 0.75) -> List[Dict[str, Any]]:
        """
            ...
            async with get_async_session() as db:
                ...
                for face in all_faces:
                    if face.embedding and isinstance(face.embedding, dict):
                        ...
                        if stored_embedding:
                            similarity = self._calculate_embedding_similarity(face_embedding, stored_embedding)
    ...
    async def generate_face_suggestions(self, unassigned_face_ids: List[str]) -> List[Dict[str, Any]]:
        """
        ...
        try:
            async with get_async_session() as db:
                for face_id in unassigned_face_ids:
                    ...
                    embedding_data = face.embedding.get('data', []) if isinstance(face.embedding, dict) else []
                    ...
                    if similar_faces:
                        ...
                        if person_scores:
                            best_person_id = max(person_scores.keys(), key=lambda k: person_scores[k])
                            ...
                            if person:
                                suggestions.append({
                                    ...
                                    "confidence": min(95, int(person_scores[best_person_id] * 100)),
                                    "face_ids": [face_id]
                                })
    ...
    async def generate_face_crop(self, image_path: str, bounding_box: Tuple[int, int, int, int]) -> Optional[str]:
        """
        ...
        try:
            ...
            def _sync_generate_crop():
                ...
                with Image.open(full_image_path) as img:
                    ...
                    crop_x = max(0, min(face_center_x - crop_size // 2, img_width - crop_size))
                    crop_y = max(0, min(face_center_y - crop_size // 2, img_height - crop_size))
                    ...
                    # Extract face crop
                    face_crop = img.crop((crop_x, crop_y, crop_x + crop_size, crop_y + crop_size))
                    ...
                    if avg_brightness < 40:
                        ...
                        large_crop_x = max(0, min(face_center_x - large_crop_size // 2, img_width - large_crop_size))
                        large_crop_y = max(0, min(face_center_y - large_crop_size // 2, img_height - large_crop_size))
                        
                        face_crop = img.crop((large_crop_x, large_crop_y,
                                            large_crop_x + large_crop_size, large_crop_y + large_crop_size))
                        face_crop = face_crop.resize((200, 200), Image.Resampling.LANCZOS)
            ...
            crop_filename = f"face_crop_{timestamp}_{hash(str(bounding_box)) % 10000:04d}.jpg"
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            library_available = await self.initialize_face_recognition()
            ...
            status = "healthy" if (library_available and can_detect and can_encode) else "degraded"
