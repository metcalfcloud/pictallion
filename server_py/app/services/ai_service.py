...
class AIService:
    ...
    async def analyze_image(
        self, image_path: str, preferred_provider: Optional[str] = None
    ) -> AIMetadata:
        """
        ...
        """
        return await self.analyze_image_with_people_context(
            image_path, preferred_provider, []
        )
        ...
        try:
            ...
            if provider in ["openai", "both"] and self.config.openai["api_key"]:
                ...
                try:
                    ...
                    if "429" in str(e) or "quota" in str(e).lower():
                        logger.warning(
                            "OpenAI quota exceeded, falling back to alternative processing"
                        )
            else:
                logger.debug(
                    f"OpenAI not available - provider: {provider}, has_key: {bool(self.config.openai['api_key'])}"
                )
            ...
            if provider in ["ollama", "both"]:
                ...
                if is_available:
                    ...
                    return await self._generate_basic_metadata(
                        image_path, people_context
                    )
    ...
    async def _analyze_with_openai(
        self, image_path: str, people_context: List[PersonContext]
    ) -> AIMetadata:
...
For events, detect holidays, celebrations, activities, and life events.{people_context_str}"""
        ...
        try:
            ...
            return AIMetadata(
                ...
                short_description=ai_result.get(
                    "shortDescription", "OpenAI analysis unavailable"
                ),
                long_description=ai_result.get(
                    "longDescription",
                    "Detailed OpenAI analysis unavailable for this image.",
                ),
                ...
                ai_confidence_scores=ai_result.get(
                    "aiConfidenceScores",
                    {
                        "tags": 0.8,
                        "description": 0.8,
...
- Return ONLY valid JSON, no additional text{people_context_str}"""
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                return AIMetadata(
                    ai_tags=ai_result.get("aiTags", [])[:8],
                    short_description=ai_result.get(
                        "shortDescription", "AI analysis unavailable"
                    ),
                    long_description=ai_result.get(
                        "longDescription",
                        "Detailed AI analysis unavailable for this image.",
                    ),
                    detected_objects=ai_result.get("detectedObjects", []),
                    place_name=ai_result.get("placeName"),
                    perceptual_hash=await self._generate_perceptual_hash(image_path),
                    ai_confidence_scores=ai_result.get(
                        "aiConfidenceScores",
                        {"tags": 0.5, "description": 0.5, "objects": 0.5, "place": 0.0},
                    ),
                )
    ...
    async def _generate_basic_metadata(
        self, image_path: str, people_context: List[PersonContext]
    ) -> AIMetadata:
        ...
        if people_context:
            ...
            # Add age-based tags
            children = [
                p for p in people_context if p.age_in_photo and p.age_in_photo < 13
            ]
            teens = [
                p
                for p in people_context
                if p.age_in_photo and 13 <= p.age_in_photo < 18
            ]
            adults = [
                p
                for p in people_context
                if p.age_in_photo and 18 <= p.age_in_photo < 60
            ]
            seniors = [
                p for p in people_context if p.age_in_photo and p.age_in_photo >= 60
            ]
            ...
            for person in people_context:
                all_relationships.extend(
                    [r.get("type", "") for r in person.relationships]
                )
            ...
            long_description = (
                f"A wonderful family moment captured with {display_names}."
            )
            ...
            if aged_people and len(aged_people) <= 4:
                ...
                long_description += (
                    f" Everyone captured beautifully - {', '.join(ages_info)}."
                )
        else:
    ...
    async def generate_tags(
        self, description: str, preferred_provider: Optional[str] = None
    ) -> List[str]:
        """
        ...
        try:
            ...
            if (
                provider in ["ollama", "both"]
                and await self._check_ollama_availability()
            ):
            ...
            if provider in ["openai", "both"] and self.openai_client:
                response = await self.openai_client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": "Generate 5-8 relevant tags for the given image description. Return as JSON array of strings.",
                        },
                        {
                            "role": "user",
                            "content": f"Generate tags for: {description}",
                        },
                    ],
                    response_format={"type": "json_object"},
                    max_tokens=200,
                ...
                result = json.loads(
                    response.choices[0].message.content or '{"tags": []}'
                )
    ...
    async def enhance_metadata_with_short_description(
        self, metadata: AIMetadata, image_path: str
    ) -> AIMetadata:
        """
        ...
        try:
            ...
            if self.config.openai["api_key"] and self.config.provider in [
                "openai",
                "both",
            ]:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            status = (
                "healthy"
                if (providers["openai"] or providers["ollama"])
                else "degraded"
            )
