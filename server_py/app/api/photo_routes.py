...
from app.core.crud import FileVersionCRUD
from app.core.database import get_db
from app.models.schemas import (InsertAssetHistory, InsertFileVersion,
                                InsertMediaAsset, ProcessingStateEnum,
                                TierEnum)
from app.services.ai_service import AIService
from app.services.face_detection_service import FaceDetectionService
from app.services.file_manager_service import FileManagerService
from app.services.metadata_service import MetadataEmbeddingService
...
async def get_metadata_service() -> MetadataEmbeddingService:
    """Get metadata service instance."""
    return MetadataEmbeddingService()
...
async def list_photos(
    tier: Optional[str] = Query(
        None, description="Filter by tier: silver, gold, unprocessed, all_versions"
    ),
    ...
    try:
        ...
        if tier == "unprocessed":
            ...
            photos = await crud.get_photos_by_tier(
                tier=tier, show_superseded=show_all_versions, limit=limit, offset=offset
            )
        else:
        ...
        enhanced_photos = []
        for photo in photos:
            media_asset = await crud.get_media_asset(photo.originalFilename)
        ...
        return PhotoListResponse(
            photos=enhanced_photos, total_count=total_count, page=page, limit=limit
        )
...
async def get_recent_photos(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch recent photos: {str(e)}"
        )
...
async def get_photo(
    ...
    try:
        ...
        media_asset = await crud.get_media_asset(photo.originalFilename)
        history = await crud.get_asset_history(photo.originalFilename)
...
async def upload_photos(
    ...
    metadata_service: MetadataEmbeddingService = Depends(get_metadata_service),
        ...
        for file in files:
            try:
                ...
                if duplicate_conflicts:
                    conflicts.extend(
                        [
                            {**conflict, "filename": file.filename}
                ...
                media_asset = await crud.create_media_asset(
                    InsertMediaAsset(originalFilename=file.filename)
                )

                # Process file to Silver tier
                silver_path = await file_manager.process_to_silver(
                ...
                # Create file version
                file_version = await crud.create_file_version(
                    InsertFileVersion(
                        originalFilename=media_asset.id,
                        tier=TierEnum.SILVER,
                        originalFilename=silver_path,
                        originalFilename=originalFilename,
                        originalFilename=file.size,
                        originalFilename=file.content_type,
                        metadata=metadata,
                        originalFilename=None,  # No AI processing at upload
                ...
                if file.content_type.startswith("image/"):
                    ...
                    if faces:
                        ...
                        await crud.update_file_version(
                            file_version.id, {"metadata": combined_metadata}
                        )

                # Log ingestion
                await crud.create_asset_history(
                    InsertAssetHistory(
                        originalFilename=media_asset.id,
                        action="INGESTED",
...
async def get_filename_preview(
    ...
    try:
        ...
        preview_filename = await ai_service.generate_filename(
            photo=photo, media_asset=media_asset, pattern=final_pattern, tier="gold"
        )
...
async def get_photo_versions(
    ...
    try:
        ...
        versions = await crud.get_file_versions_by_asset(photo.originalFilename)

        # Enhance with media asset info and sort by tier priority
        enhanced_versions = []
        for version in versions:
            media_asset = await crud.get_media_asset(version.originalFilename)
        ...
        enhanced_versions.sort(
            key=lambda v: tier_priority.get(v["tier"], 0), reverse=True
        )
...
async def process_photo_ai(
    ...
    try:
        ...
        if photo.tier != "silver":
            raise HTTPException(
                status_code=400, detail="Only Silver tier photos can be AI processed"
            )

        if not photo.originalFilename.startswith("image/"):
            raise HTTPException(
                status_code=400, detail="AI processing only supports images"
            )
        ...
        if existing_ai.get("shortDescription"):
            raise HTTPException(
                status_code=400, detail="Photo already has AI processing"
            )
        ...
        ai_metadata = await ai_service.analyze_image_with_context(
            photo.originalFilename, people_context
        )
...
async def promote_photo(
    ...
    try:
        ...
        if photo.tier != "silver":
            raise HTTPException(
                status_code=400, detail="Photo must be in Silver tier for promotion"
            )

        # Copy file to Gold tier
        media_asset = await crud.get_media_asset(photo.originalFilename)
        gold_path = await file_manager.copy_to_gold(photo.originalFilename, media_asset)

        # Create Gold file version
        gold_version = await crud.create_file_version(
            InsertFileVersion(
                originalFilename=photo.originalFilename,
                tier=TierEnum.GOLD,
                originalFilename=gold_path,
                originalFilename=photo.originalFilename,
                originalFilename=photo.originalFilename,
                originalFilename=photo.originalFilename,
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to promote photo: {str(e)}"
        )
            ...
            try:
                ...
                if request.operation == "ai_process":
                    ...
                    if photo.tier == "silver":
                        # Promote to gold
                        media_asset = await crud.get_media_asset(photo.originalFilename)
                        gold_path = await file_manager.copy_to_gold(
                            photo.originalFilename, media_asset
                        )

                        await crud.create_file_version(
                            InsertFileVersion(
                                originalFilename=photo.originalFilename,
                                tier=TierEnum.GOLD,
                                originalFilename=gold_path,
...
async def update_photo_rating(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update rating: {str(e)}"
        )
...
async def get_burst_analysis(
    ...
    try:
        ...
        return {"groups": [], "total_photos": len(photos), "burst_groups": 0}
