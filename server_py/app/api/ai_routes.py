...
from app.core.crud import file_version
from app.core.database import get_db
from app.models.schemas import AICategoryEnum, AIProviderEnum, InsertAIPrompt
...
async def update_ai_config(
    request: AIConfigRequest, ai_service: AIService = Depends(get_ai_service)
) -> Dict[str, Any]:
    ...
    try:
        ...
        ai_service.set_config(config_updates)

        return {"success": True, "message": "AI configuration updated"}

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to update AI config: {str(e)}"
        )
...
async def test_ai_providers(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to test AI providers: {str(e)}"
        )
...
async def analyze_image(
    ...
    try:
        ...
        if not photo.mime_type.startswith("image/"):
            raise HTTPException(
                status_code=400, detail="AI analysis only supports images"
            )
        ...
        if request.include_people_context:
            ...
            people_context: List[Dict[str, Any]] = (
                await ai_service.build_people_context(faces, crud)
            )
        ...
        return AIAnalysisResponse(
            success=True,
            analysis=enhanced_analysis if isinstance(enhanced_analysis, dict) else {},
            confidence_scores=(
                enhanced_analysis.get("ai_confidence_scores", {})
            ...
            try:
                ...
                if people_context:
                    analysis = await ai_service.analyze_image(
                        photo.file_path, analysis_provider
                    )
                else:
                    analysis = await ai_service.analyze_image(
                        photo.file_path, analysis_provider
                ...
                enhanced_analysis = (
                    await ai_service.enhance_metadata_with_short_description(
                        analysis, photo.file_path
                    )
                )
                ...
                await crud.update_file_version(
                    photo_id,
                    {
                        ...
                        "ai_short_description": (
                            enhanced_analysis.get("short_description")
                            if isinstance(enhanced_analysis, dict)
                            else None
                        ),
                        "is_reviewed": False,
...
async def list_ai_prompts(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch AI prompts: {str(e)}"
        )


@router.get("/prompts/category/{category}", response_model=List[AIPromptResponse])
async def get_prompts_by_category(
    category: str, db: AsyncSession = Depends(get_db), crud=Depends(get_ai_crud)
) -> List[AIPromptResponse]:
...
async def get_prompts_by_provider(
    provider: str, db: AsyncSession = Depends(get_db), crud=Depends(get_ai_crud)
) -> List[AIPromptResponse]:
...
async def get_ai_prompt(
    prompt_id: str, db: AsyncSession = Depends(get_db), crud=Depends(get_ai_crud)
) -> AIPromptResponse:
    """Get AI prompt by ID."""
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch AI prompt: {str(e)}"
        )
...
async def create_ai_prompt(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create AI prompt: {str(e)}"
        )
...
async def update_ai_prompt(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update AI prompt: {str(e)}"
        )
...
async def delete_ai_prompt(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to delete AI prompt: {str(e)}"
        )


@router.post("/prompts/reset-to-defaults")
async def reset_prompts_to_defaults(
    db: AsyncSession = Depends(get_db), crud=Depends(get_ai_crud)
) -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to reset prompts: {str(e)}"
        )
...
async def get_default_prompts(
    ...
    try:
        ...
        return {"prompts": defaults, "categories": categories}

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch default prompts: {str(e)}"
        )
...
async def generate_ai_filename(
    ...
    try:
        ...
        filename = (
            f"{pattern}_{photo_id}_{tier}"  # Fallback: generate filename manually
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate filename: {str(e)}"
        )
...
async def enhance_description(
    ...
    try:
        ...
        enhanced = (
            f"{base_description} (enhanced)"  # Fallback: manually enhance description
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to enhance description: {str(e)}"
        )
