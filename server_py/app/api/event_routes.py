...
class EventMatch(BaseModel):
    ...
    event_type: str = Field(..., alias="eventType")  # "holiday", "birthday", "custom"
    confidence: int
...
class CreateEventRequest(BaseModel):
    ...
    recurring_type: Optional[str] = Field(None, alias="recurringType")  # "yearly", "monthly", "weekly"
    person_id: Optional[str] = Field(None, alias="personId")
...
async def get_events(
    type_filter: Optional[str] = Query(None, alias="type", description="Filter by event type"),
    enabled_only: bool = Query(True, alias="enabledOnly", description="Only return enabled events"),
    ...
    try:
        ...
        if enabled_only:
            filtered_events = [e for e in filtered_events if e.is_enabled]
        ...
        for event in filtered_events:
            response_events.append(EventResponse(
                ...
                date=event.date.isoformat() if isinstance(event.date, datetime) else event.date,
                type=event.type,
...
async def update_event(
    ...
    try:
        ...
        if updates.date is not None:
            try:
                update_data["date"] = datetime.fromisoformat(updates.date.replace('Z', '+00:00'))
            except ValueError:
                raise HTTPException(status_code=400, detail="Invalid date format. Use ISO format.")
        ...
        if updates.recurring_type is not None:
            update_data["recurring_type"] = updates.recurring_type
        if updates.person_id is not None:
            update_data["person_id"] = updates.person_id
        ...
        return EventResponse(
            ...
            isEnabled=updated_event.is_enabled,
            isRecurring=updated_event.is_recurring,
            recurringType=updated_event.recurring_type,
...
async def get_holiday_sets(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch holiday sets: {str(e)}")
...
async def detect_events(
    ...
    try:
        # Parse photo date
        try:
            photo_date = datetime.fromisoformat(request.photo_date.replace('Z', '+00:00'))
        except ValueError:
            raise HTTPException(status_code=400, detail="Invalid photo date format. Use ISO format.")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to detect events: {str(e)}")


@router.post("/people/{person_id}/age-in-photo", response_model=AgeCalculationResponse)
async def calculate_age_in_photo(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to calculate age: {str(e)}")


@router.get("/events/by-type/{event_type}", response_model=List[EventResponse])
async def get_events_by_type(
    ...
    try:
        events = await crud.get_events_by_type(event_type)
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch events by type: {str(e)}")
...
async def get_event_statistics(
    ...
    try:
        ...
        upcoming_events = []
        for event in events:
            if event.is_enabled and event.date >= now and event.date <= future_date:
    ...
    try:
        ...
        for event_id in event_ids:
            try:
                event = await crud.get_event(event_id)
                if event:
                    await crud.update_event(event_id, {"is_enabled": enabled})
                    updated_count += 1
                else:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to bulk toggle events: {str(e)}")
...
async def get_event_clusters(
    ...
    try:
        ...
        for photo in photos:
            if photo.event_type and photo.event_name:
                key = f"{photo.event_type}:{photo.event_name}"
        ...
        for group_data in event_groups.values():
            if group_data["dates"]:
                ...
                clusters.append(EventCluster(
                    eventType=group_data["event_type"],
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get event clusters: {str(e)}")
    ...
    try:
        ...
        for photo in photos:
            try:
                ...
                if not force_redetect and photo.event_type:
                    continue
                ...
                if photo.metadata and isinstance(photo.metadata, dict):
                    ...
                    if exif.get("dateTimeOriginal"):
                        try:
                            photo_date = datetime.fromisoformat(exif["dateTimeOriginal"])
                        except:
                            pass
                ...
                if detected_events:
                    ...
                    if best_event["confidence"] >= 80:  # Only use high-confidence matches
                        # Update photo with event data
                        await crud.update_file_version(photo.id, {
        ...
        raise HTTPException(status_code=500, detail=f"Failed to auto-detect events: {str(e)}")
...
async def import_holiday_events(
    ...
    try:
        ...
        for country_code in country_codes:
            ...
            holiday_set = next((hs for hs in holiday_sets if hs["code"] == country_code), None)
            if not holiday_set:
                errors.append(f"Holiday set not found for country: {country_code}")
                continue
            
            # Get holidays for this country and year
            holidays = event_service.get_holidays_for_country_and_year(country_code, year)
            ...
            for holiday in holidays:
                try:
                    ...
                    existing = next((e for e in existing_events if e.name == event_name), None)
                    ...
                    if existing and overwrite_existing:
                        ...
                        await crud.update_event(existing.id, {
                            "name": event_name,
        ...
        raise HTTPException(status_code=500, detail=f"Failed to import holidays: {str(e)}")