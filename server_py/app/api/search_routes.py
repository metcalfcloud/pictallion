...
from app.core.crud import media_asset
...
class SearchFilters(BaseModel):
    ...
    file_size: Optional[Dict[str, int]] = Field(
        None, alias="fileSize"
...
async def advanced_photo_search(
    ...
    crud=Depends(get_media_asset_crud),
    ...
    try:
        ...
        search_params = {
            ...
            "sort": (
                request.sort.dict()
                if request.sort
                else {"field": "created_at", "direction": "desc"}
            ),
            "limit": request.limit,
...
async def simple_photo_search(
    ...
    rating_min: Optional[int] = Query(
        None, description="Minimum rating", alias="ratingMin"
    ),
    rating_max: Optional[int] = Query(
        None, description="Maximum rating", alias="ratingMax"
    ),
    ...
    try:
        ...
        if q:
            results = await search_service.search_photos_with_query(
                query=q, filters=filters, limit=limit, offset=offset
            )
        else:
            results = await search_service.search_photos_by_filters(
...
async def find_similar_photos(
    ...
    threshold: float = Query(
        85, ge=50, le=99, description="Similarity threshold (50-99)"
    ),
    ...
    try:
        ...
        similarity_scores = [
            photo.get("similarity_score", 0) for photo in similar_photos
        ]
        ...
        reference_photo_dict["media_asset"] = (
            media_asset.dict() if media_asset else None
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to find similar photos: {str(e)}"
        )
...
async def get_search_suggestions(
    ...
    type: str = Query(
        "all", description="Suggestion type: all, tags, people, locations"
    ),
    ...
    try:
        ...
        return {"query": q, "suggestions": suggestions, "type": type}
...
async def save_search(
    ...
    try:
        ...
        smart_rules = {"rules": [], "operator": "AND"}
...
async def get_saved_searches(
    db: AsyncSession = Depends(get_db), crud=Depends(get_media_asset_crud)
):
    """
...
async def analyze_photo_similarity(
    ...
    try:
        ...
        similar_groups = await search_service.find_similar_photo_groups(
            photos=photos, threshold=threshold
        )
...
async def update_smart_collections(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update smart collections: {str(e)}"
        )
...
async def search_tags(
    ...
    try:
        ...
        matching_tags = [tag for tag in all_tags if q.lower() in tag["tag"].lower()]
...
async def get_photos_by_tag(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get photos by tag: {str(e)}"
        )
