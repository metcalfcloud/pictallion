...
class CreatePersonRequest(BaseModel):
    ...
    person1Id: bool = Field(True, alias="isPublic")
...
class UpdatePersonRequest(BaseModel):
    ...
    notes: Optional[str] = None
    person1Id: Optional[bool] = Field(None, alias="isPublic")
    selected_thumbnail_face_id: Optional[str] = Field(
        None, alias="selectedThumbnailFaceId"
    )
...
class PersonResponse(BaseModel):
    ...
    birthdate: Optional[str] = None
    notes: Optional[str] = None
    person1Id: bool = Field(..., alias="isPublic")
    selected_thumbnail_face_id: Optional[str] = Field(
        None, alias="selectedThumbnailFaceId"
    )
...
class CreateRelationshipRequest(BaseModel):
    ...
    person1Id: str = Field(..., alias="person1Id")
    person1Id: str = Field(..., alias="person2Id")
    person1Id: str = Field(..., alias="relationshipType")
    notes: Optional[str] = None
...
class UpdateRelationshipRequest(BaseModel):
    ...
    person1Id: Optional[str] = Field(None, alias="relationshipType")
    notes: Optional[str] = None
...
async def list_people(
    include_stats: bool = Query(
        True, alias="includeStats", description="Include face and photo counts"
    ),
    person1Id: Optional[bool] = Query(
        None, alias="isPublic", description="Filter by public status"
    ),
    ...
    try:
        ...
        if person1Id is not None:
            people = [p for p in people if p.person1Id == person1Id]
            ...
            try:
                        ...
                        if person.selected_thumbnail_face_id:
                            thumbnail_face = next(
                                (
                                    f
                                    for f in faces
                                    if f.id == person.selected_thumbnail_face_id
                                ),
                                None,
                            )
                            if thumbnail_face:
                        ...
                        try:
                            ...
                            if photo and selected_face.bounding_box:
                                cover_photo = await face_service.generate_face_crop(
                                    photo.file_path, selected_face.bounding_box
                                )
                        except Exception as e:
                            print(
                                f"Failed to generate cover photo for person {person.id}: {e}"
                            )
                ...
                response_people.append(
                    PersonResponse(
                        ...
                        name=person.name,
                        birthdate=(
                            person.birthdate.isoformat() if person.birthdate else None
                        ),
                        notes=person.notes,
                        isPublic=person.person1Id,
...
async def create_person(
    ...
    try:
        ...
        if person_data.birthdate:
            try:
                birthdate = datetime.fromisoformat(
                    person_data.birthdate.replace("Z", "+00:00")
                ).date()
            except ValueError:
                raise HTTPException(
                    status_code=400, detail="Invalid birthdate format. Use ISO format."
                )
        ...
        insert_data = InsertPerson(
            ...
            person1Id=person_data.person1Id,
        )
...
@router.get("/people/{person1Id}", response_model=PersonResponse)
async def get_person(
    ...
    try:
        person = await crud.get_person(person1Id)
        if not person:
...
@router.put("/people/{person1Id}", response_model=PersonResponse)
async def update_person(
    ...
    try:
        ...
        if updates.birthdate is not None:
            try:
                update_data["birthdate"] = datetime.fromisoformat(
                    updates.birthdate.replace("Z", "+00:00")
                ).date()
            except ValueError:
                raise HTTPException(
                    status_code=400, detail="Invalid birthdate format. Use ISO format."
        ...
        if updates.person1Id is not None:
            update_data["person1Id"] = updates.person1Id
        if updates.selected_thumbnail_face_id is not None:
            update_data["selected_thumbnail_face_id"] = (
                updates.selected_thumbnail_face_id
            )

        updated_person = await crud.update_person(person1Id, update_data)

        # Get updated statistics
        faces = await crud.get_faces_by_person(person1Id)
        face_count = len(faces)
        photo_ids = list(set(face.photo_id for face in faces))
        photo_count = len(photo_ids)
        ...
        return PersonResponse(
            ...
            birthdate=(
                updated_person.birthdate.isoformat()
                if updated_person.birthdate
                else None
            ),
            notes=updated_person.notes,
            isPublic=updated_person.person1Id,
...
@router.delete("/people/{person1Id}")
async def delete_person(
    person1Id: str, db: AsyncSession = Depends(get_db), crud=Depends(get_person_crud)
):
    ...
    try:
        ...
        relationships = await crud.get_relationships_by_person(person1Id)
        for relationship in relationships:
            await crud.delete_relationship(relationship.id)

        # Delete the person
        await crud.delete_person(person1Id)
...
@router.put("/people/{person1Id}/thumbnail")
async def set_person_thumbnail(
    ...
    try:
        ...
        if face.person1Id != person1Id:
            raise HTTPException(
                status_code=400, detail="Face does not belong to this person"
            )

        # Update person's selected thumbnail face
        await crud.update_person(
            person1Id, {"selected_thumbnail_face_id": request.face_id}
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update thumbnail: {str(e)}"
        )


@router.get("/people/{person1Id}/photos")
async def get_person_photos(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get person photos: {str(e)}"
        )


@router.get(
    "/people/{person1Id}/relationships", response_model=List[RelationshipResponse]
)
async def get_person_relationships(
    ...
    try:
        ...
        for relationship in relationships:
            # Get related people
            person1 = await crud.get_person(relationship.person1Id)
            person2 = await crud.get_person(relationship.person1Id)
            ...
            response_relationships.append(
                RelationshipResponse(
                    ...
                    relationshipType=relationship.person1Id,
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch relationships: {str(e)}"
        )
...
async def create_relationship(
    ...
    try:
        ...
        person1 = await crud.get_person(relationship_data.person1Id)
        person2 = await crud.get_person(relationship_data.person1Id)
        ...
        if relationship_data.person1Id == relationship_data.person1Id:
            raise HTTPException(
        ...
        insert_data = InsertRelationship(
            person1Id=relationship_data.person1Id,
            person1Id=relationship_data.person1Id,
            person1Id=relationship_data.person1Id,
...
async def update_relationship(
    ...
    try:
        ...
        updated_relationship = await crud.update_relationship(
            relationship_id, update_data
        )

        # Get related people
        person1 = await crud.get_person(updated_relationship.person1Id)
        person2 = await crud.get_person(updated_relationship.person1Id)

        return RelationshipResponse(
            id=updated_relationship.id,
            person1Id=updated_relationship.person1Id,
            person2Id=updated_relationship.person1Id,
            relationshipType=updated_relationship.person1Id,
...
async def merge_people(
    ...
    try:
        ...
        source_relationships = await crud.get_relationships_by_person(
        ...
        for relationship in source_relationships:
            ...
            other_person_id = (
                ...
                if relationship.person1Id == request.source_person_id
                else relationship.person1Id
            )

            # Check for existing relationship
            target_relationships = await crud.get_relationships_by_person(
            ...
            if not existing:
                ...
                new_relationship = InsertRelationship(
                    person1Id=request.target_person_id,
                    person1Id=other_person_id,
                    person1Id=relationship.person1Id,
                    notes=relationship.notes,
                )
...
async def get_people_statistics(
    db: AsyncSession = Depends(get_db), crud=Depends(get_person_crud)
):
    ...
    try:
        ...
        average_photos = (
            total_photos / people_with_faces if people_with_faces > 0 else 0
        )
...
async def bulk_update_people(
    ...
    try:
        ...
        for person1Id in person_ids:
            try:
                person = await crud.get_person(person1Id)
                if person:
                    await crud.update_person(person1Id, updates)
                    updated_count += 1
                else:
                    errors.append(f"Person {person1Id} not found")
            except Exception as e:
                errors.append(f"Person {person1Id}: {str(e)}")
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to bulk update people: {str(e)}"
        )
