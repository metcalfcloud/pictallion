...
async def serve_media_file(
    ...
    quality: Optional[str] = Query(
        None, description="Thumbnail quality: low, medium, high"
    ),
    ...
    try:
        ...
        if w or h or quality:
            try:
                ...
                thumbnail_quality = {"low": 60, "medium": 80, "high": 90}.get(
                    quality, 80
                )
                ...
                thumbnail_path = await thumbnail_service.generate_thumbnail(
                    str(file_path), thumbnail_options
                )
        ...
        return FileResponse(path=str(file_path), headers=headers, filename=filename)
...
async def serve_temp_file(filename: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to serve temp file: {str(e)}"
        )
...
async def stream_media_file(tier: str, date: str, filename: str, request: Request):
    """
