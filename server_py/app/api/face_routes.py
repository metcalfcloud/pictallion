...
async def get_faces_for_photo(
    ...
    try:
        ...
        for face in faces:
            ...
            if face.person_id:
                ...
                if person and person.birthdate:
                    ...
                    photo_with_asset = {**photo.dict(), "media_asset": media_asset.dict() if media_asset else None}
                    photo_date = await face_service.extract_photo_date(photo_with_asset)
                    
                    if photo_date:
                        face_dict["age_in_photo"] = await face_service.calculate_age_in_photo(
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch faces for photo: {str(e)}")
...
async def get_unassigned_faces(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch unassigned faces: {str(e)}")
...
async def get_ignored_faces(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to fetch ignored faces: {str(e)}")
        ...
        for face in unassigned_faces:
            ...
            for similar in similar_faces:
                if similar.get("person_id"):
                    ...
                    person_matches[person_id]["total_similarity"] += similar.get("similarity", 0)
            ...
            for person_id, match in person_matches.items():
                ...
                if confidence >= 75:
                    ...
                    if person.selected_thumbnail_face_id:
                        representative_face_url = f"/api/faces/{person.selected_thumbnail_face_id}/crop"
            ...
            if face_suggestions:
                ...
                suggestions.append(FaceSuggestionsResponse(
                    face_id=face.id,
                    suggestions=face_suggestions[:3]
                ))
        
        return suggestions
        
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to get face suggestions: {str(e)}")
    ...
    try:
        ...
        for face in unassigned_faces:
            ...
            for similar in similar_unassigned:
                similar_face = next((uf for uf in unassigned_faces if uf.id == similar.get("id")), None)
            ...
            if len(group_faces) >= min_group_size:
                ...
                if similar_unassigned:
                    avg_confidence = round(
                        sum(sf.get("similarity", 0) for sf in similar_unassigned) / len(similar_unassigned) * 100
                    )
                ...
                if similar_unassigned and similar_unassigned[0].get("similarity", 0) >= 0.97:
                    unassigned_groups.append({
                        "type": "similarity",
                        "group_id": f"similar_{face.id}",
                        "group_name": f"Possible Match ({round(similar_unassigned[0].get('similarity', 0) * 100)}% confident)",
                        "faces": [f.dict() for f in group_faces],
                        "avg_confidence": round(similar_unassigned[0].get('similarity', 0) * 100)
                    })
                else:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get grouped faces: {str(e)}")
...
async def unignore_face(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to unignore face: {str(e)}")
...
async def bulk_ignore_faces(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to bulk ignore faces: {str(e)}")
...
async def bulk_unignore_faces(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to bulk unignore faces: {str(e)}")
...
async def batch_assign_faces(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to batch assign faces: {str(e)}")
...
async def reprocess_unassigned_faces(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to reprocess faces: {str(e)}")
...
async def get_face_crop(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to generate face crop: {str(e)}")