...
class TestPhotoUploadAndProcessingWorkflow:
    ...
    async def test_full_photo_upload_pipeline(self, async_client, db_session, temp_dir):
        ...
        with (
            ...
            patch(
                "app.services.duplicate_detection_service.duplicate_detection_service.check_for_duplicates"
            ) as mock_duplicates,
        ):
    ...
    async def test_batch_photo_processing_workflow(
        self, async_client, db_session, temp_dir
    ):
        ...
        with patch("app.services.ai_service.ai_service.analyze_image") as mock_ai:
            ...
            for i, image_path in enumerate(image_paths):
                files_data.append(
                    (
                        "files",
                        (f"batch_photo_{i}.jpg", open(image_path, "rb"), "image/jpeg"),
                    )
                )
            ...
            try:
                ...
                batch_response = await async_client.post(
                    "/api/ai/batch-analyze", json=batch_data
                )
...
class TestPeopleManagementWorkflow:
    ...
    async def test_person_identification_workflow(
        self, async_client, db_session, temp_dir
    ):
        """Test person identification and photo association workflow."""
        # Step 1: Create a person
        person_data = {"name": "John Doe", "notes": "Test person for workflow"}
        ...
        for i in range(3):
            ...
            with patch(
                ...
                mock_faces.return_value = [
                    {
                        ...
                        "encoding": [0.2 + i * 0.01]
                        * 128,  # Slightly different encodings
                        "confidence": 90 + i,
                    }
                ]
                ...
                photos_with_faces.append(
                    {"photo_id": photo_id, "faces": face_data["faces"]}
                )
        ...
        person_photos_response = await async_client.get(
        ...
        face_search_response = await async_client.get(
            f"/api/search/faces?person_id={person_id}"
        )
    ...
    async def test_person_merging_workflow(self, async_client, db_session, temp_dir):
        ...
        for person_id, suffix in [(person1_id, "a"), (person2_id, "b")]:
            for i in range(2):
                ...
                face = await DatabaseTestHelper.create_test_face(
                    db_session, file_version=photo, person_id=person_id
                )
        ...
        merge_data = {"source_person_id": person2_id, "target_person_id": person1_id}
        ...
        person1_final_photos = await async_client.get(
            f"/api/people/{person1_id}/photos"
        )
        ...
        if person2_final.status_code == 200:
            person2_final_photos = await async_client.get(
                f"/api/people/{person2_id}/photos"
            )
...
class TestCollectionManagementWorkflow:
    ...
    async def test_manual_collection_workflow(self, async_client, db_session):
        ...
        collection_response = await async_client.post(
            "/api/collections/", json=collection_data
        )
        ...
        add_data = {"photo_ids": [photo.id for photo in vacation_photos]}

        add_response = await async_client.post(
            f"/api/collections/{collection_id}/photos", json=add_data
        )
    ...
    async def test_smart_collection_workflow(self, async_client, db_session):
        ...
        smart_response = await async_client.post(
            "/api/collections/", json=smart_collection_data
        )
        ...
        with patch(
            "app.services.advanced_search_service.advanced_search_service.execute_smart_collection_query"
        ) as mock_query:
            # Mock the smart collection query to return matching photos
            matching_photos = [
                photo for photo in photos[:5] if photo.rating >= 4  # vacation photos
            ]

            mock_query.return_value = {
                "items": [
                    {"id": photo.id, "rating": photo.rating}
...
class TestSearchWorkflow:
    ...
    async def test_comprehensive_search_workflow(self, async_client, db_session):
        ...
        advanced_response = await async_client.post(
            "/api/search/advanced", json=advanced_criteria
        )
    ...
    async def test_search_with_filters_workflow(self, async_client, db_session):
        ...
        for i in range(12):  # 12 months of photos
            ...
            photo = await DatabaseTestHelper.create_test_file_version(
                ...
                rating=(i % 5) + 1,
                metadata={"exif": {"date_taken": month_date.isoformat()}},
            )
        ...
        date_response = await async_client.post(
            "/api/search/advanced", json=date_search_params
        )
...
class TestAIWorkflow:
    ...
    async def test_ai_analysis_pipeline(self, async_client, db_session, temp_dir):
        ...
        with (
            ...
            patch(
                "app.services.ai_service.ai_service.generate_description"
            ) as mock_desc,
        ):
            ...
            assert any(
                tag in combined_keywords for tag in ["nature", "landscape", "outdoor"]
            )
...
class TestPerformanceWorkflows:
    ...
    async def test_bulk_operations_performance(self, async_client, db_session):
        ...
        bulk_update_data = {
            "photo_ids": photo_ids,
            "updates": {"rating": 5, "keywords": ["bulk_updated"]},
        }

        bulk_response = await async_client.post(
            "/api/photos/bulk-update", json=bulk_update_data
        )
    ...
    async def test_ai_service_failure_recovery(self, async_client, db_session):
        ...
        with patch("app.services.ai_service.ai_service.analyze_image") as mock_ai:
            ...
            assert response.status_code in [
                500,
                503,
            ]  # Internal error or service unavailable
