...
class TestPhotoRoutes:
    ...
    async def test_list_photos_with_filters(self, async_client: AsyncClient, db_session):
...
class TestPeopleRoutes:
    ...
    async def test_merge_people(self, async_client: AsyncClient, db_session):
        ...
        merge_data = {
            "source_person_id": person2.id,
            "target_person_id": person1.id
        }
...
class TestCollectionRoutes:
    ...
    async def test_add_photos_to_collection(self, async_client: AsyncClient, db_session):
        ...
        response = await async_client.post(
            f"/api/collections/{collection.id}/photos",
            json=add_data
        )
        assert response.status_code == 200
    ...
    async def test_remove_photos_from_collection(self, async_client: AsyncClient, db_session):
...
class TestFaceRoutes:
    ...
    async def test_detect_faces_in_photo(self, async_client: AsyncClient, db_session):
        ...
        with patch('app.services.face_detection_service.face_detection_service.detect_faces') as mock_detect:
    ...
    async def test_face_similarity_search(self, async_client: AsyncClient, db_session):
        ...
        with patch('app.services.face_detection_service.face_detection_service.find_similar_faces') as mock_similar:
...
class TestFileRoutes:
    ...
    async def test_upload_multiple_files(self, async_client: AsyncClient, temp_dir):
        ...
        for i in range(3):
            ...
            files_data.append(("files", (f"test_{i}.jpg", open(image_path, "rb"), "image/jpeg")))
        
        try:
            response = await async_client.post("/api/files/upload-multiple", files=files_data)
            assert response.status_code == 201
    ...
    async def test_get_thumbnail(self, async_client: AsyncClient, db_session):
        ...
        with patch('app.services.thumbnail_service.thumbnail_service.get_thumbnail_path') as mock_thumbnail:
            mock_thumbnail.return_value = "/path/to/thumbnail.jpg"
            
            response = await async_client.get(f"/api/files/{photo.id}/thumbnail?size=300")
            assert response.status_code == 200
    ...
    async def test_delete_file(self, async_client: AsyncClient, db_session):
        ...
        with patch('app.services.file_manager_service.file_manager_service.delete_file') as mock_delete:
...
class TestEventRoutes:
    ...
    async def test_detect_events(self, async_client: AsyncClient, db_session):
        ...
        with patch('app.services.event_detection_service.event_detection_service.detect_events_in_photos') as mock_detect:
            mock_detect.return_value = [
                {
                    "event_type": "holiday",
