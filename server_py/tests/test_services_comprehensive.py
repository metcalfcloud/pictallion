...
from app.services.burst_detection_service import (BurstDetectionService,
...
from app.services.location_clustering_service import (
    LocationClusteringService, location_clustering_service)
...
from tests.utils import (DatabaseTestHelper, FileTestHelper, MockServiceHelper,
...
class TestFaceDetectionService:
    ...
    async def test_detect_faces(self, face_service_instance, sample_image_with_exif):
        ...
        with (
            ...
            faces = await face_service_instance.detect_faces(
    ...
    @pytest.mark.asyncio
    async def test_generate_face_encoding(
        self, face_service_instance, sample_image_path
    ):
        ...
        with (
            ...
            encoding = await face_service_instance.generate_face_encoding(
...
class TestFileManagerService:
    ...
    async def test_save_file(
        self, file_service_instance, sample_image_path, temp_media_dir
    ):
        ...
        with (
            ...
            result_path = await file_service_instance.save_file(
                str(sample_image_path), str(temp_media_dir / "silver"), "test_photo.jpg"
            )
...
class TestThumbnailService:
    ...
    async def test_generate_thumbnails(
        self, thumbnail_service_instance, sample_image_path, temp_dir
    ):
        ...
        with (
            ...
            thumbnails = await thumbnail_service_instance.generate_thumbnails(
                str(sample_image_path), str(temp_dir), sizes=[150, 300, 600]
            )
    ...
    async def test_resize_image(
        ...
        with patch("PIL.Image.open") as mock_open:
            ...
            result = await thumbnail_service_instance.resize_image(
                str(sample_image_path), str(output_path), width=400, height=300
            )
...
class TestMetadataService:
    ...
    async def test_extract_metadata(
        self, metadata_service_instance, sample_image_with_exif
    ):
        """Test metadata extraction from image with EXIF."""
        metadata = await metadata_service_instance.extract_metadata(
            str(sample_image_with_exif)
        )
    ...
    @pytest.mark.asyncio
    async def test_extract_exif_data(
        self, metadata_service_instance, sample_image_with_exif
    ):
        """Test EXIF data extraction."""
        exif_data = await metadata_service_instance.extract_exif_data(
            str(sample_image_with_exif)
        )
    ...
    @pytest.mark.asyncio
    async def test_generate_ai_metadata(
        self, metadata_service_instance, sample_image_path
    ):
    ...
    @pytest.mark.asyncio
    async def test_image_without_exif(
        self, metadata_service_instance, sample_image_path
    ):
...
class TestDuplicateDetectionService:
    ...
    @pytest.mark.asyncio
    async def test_calculate_perceptual_hash(
        self, duplicate_service_instance, sample_image_path
    ):
    ...
    @pytest.mark.asyncio
    async def test_hash_generation_consistency(
        self, duplicate_service_instance, sample_image_path
    ):
        """Test that hash generation is consistent."""
        hash1 = await duplicate_service_instance.calculate_perceptual_hash(
            str(sample_image_path)
        )
        hash2 = await duplicate_service_instance.calculate_perceptual_hash(
            str(sample_image_path)
        )
...
class TestBurstDetectionService:
    ...
    async def test_detect_burst_sequences(self, burst_service_instance, db_session):
        ...
        for i in range(5):
            ...
            metadata = {"exif": {"date_taken": photo_time.isoformat()}}
        ...
        metadata = {"exif": {"date_taken": later_time.isoformat()}}
...
class TestAdvancedSearchService:
    ...
    async def test_search_by_date_range(self, search_service_instance, db_session):
        ...
        old_photo = await DatabaseTestHelper.create_test_file_version(
            db_session, metadata={"exif": {"date_taken": old_date.isoformat()}}
        )
        recent_photo = await DatabaseTestHelper.create_test_file_version(
            db_session, metadata={"exif": {"date_taken": recent_date.isoformat()}}
        )
    ...
    async def test_search_by_location(self, search_service_instance, db_session):
        ...
        results = await search_service_instance.search_by_location(
            db_session, latitude=40.7589, longitude=-73.9851, radius_km=1.0
        )
...
class TestEventDetectionService:
    ...
    async def test_detect_birthday_events(self, event_service_instance, db_session):
        ...
        person = await DatabaseTestHelper.create_test_person(
            db_session, name="Birthday Person", birthdate=datetime(1990, 6, 15)
        )
        ...
        birthday_photo = await DatabaseTestHelper.create_test_file_version(
            db_session, metadata={"exif": {"date_taken": birthday_2024.isoformat()}}
        )
        ...
        birthday_events = await event_service_instance.detect_birthday_events(
            db_session
        )
    ...
    async def test_detect_holiday_events(self, event_service_instance, db_session):
        ...
        new_year_photo = await DatabaseTestHelper.create_test_file_version(
            db_session, metadata={"exif": {"date_taken": "2024-01-01T00:30:00"}}
        )
...
class TestLocationClusteringService:
    ...
    @pytest.mark.asyncio
    async def test_cluster_photos_by_location(
        self, location_service_instance, db_session
    ):
        ...
        for i in range(2):
            photo = await DatabaseTestHelper.create_test_file_version(
                db_session, location=f"40.{7580+i},-73.{9853+i}"  # Times Square area
            )
    ...
    async def test_calculate_distance(self, location_service_instance):
        ...
        distance = await location_service_instance.calculate_distance(
            central_park[0], central_park[1], times_square[0], times_square[1]
        )
    ...
    @pytest.mark.asyncio
    async def test_identify_frequent_locations(
        self, location_service_instance, db_session
    ):
        ...
        frequent_locations = (
            await location_service_instance.identify_frequent_locations(
...
class TestReverseGeocodingService:
    ...
    async def test_batch_reverse_geocode(self, geocoding_service_instance):
        ...
        with patch.object(
            geocoding_service_instance, "reverse_geocode"
        ) as mock_geocode:
            ...
            results = await geocoding_service_instance.batch_reverse_geocode(
                coordinates
            )
...
class TestPromptManagementService:
    ...
    async def test_format_prompt(self, prompt_service_instance):
        ...
        template = (
            "Analyze this image: {image}. Focus on {aspect} with {detail_level} detail."
        )
        variables = {
...
class TestAINamingService:
    ...
    @pytest.mark.asyncio
    async def test_generate_photo_name(
        self, naming_service_instance, sample_image_path
    ):
    ...
    async def test_generate_collection_name(self, naming_service_instance, db_session):
        ...
        with patch.object(
            naming_service_instance, "_analyze_collection_content"
        ) as mock_analyze:
            mock_analyze.return_value = {
    ...
    async def test_suggest_person_name(self, naming_service_instance, db_session):
        ...
        person = await DatabaseTestHelper.create_test_person(
            db_session, name="Unknown Person"
        )
        ...
        with patch.object(
            naming_service_instance, "_analyze_face_context"
        ) as mock_analyze:
            mock_analyze.return_value = {
                "context_clues": [
                    "appears in family photos",
            ...
            result = await naming_service_instance.suggest_person_name(
                person.id, db_session
            )
