...
from tests.utils import (
    TestDataGenerator, DatabaseTestHelper, assert_uuid_format,
    assert_datetime_recent, TEST_TIERS, TEST_PROCESSING_STATES,
    TEST_EVENT_TYPES, TEST_RELATIONSHIP_TYPES
)
...
class TestFileVersionModel:
    ...
    async def test_file_version_ai_processing(self, db_session: AsyncSession):
        ...
        ai_description = "A beautiful sunset over the ocean with people walking on the beach"
...
class TestRelationshipModel:
    ...
    async def test_all_relationship_types(self, db_session: AsyncSession):
        ...
        person1 = await DatabaseTestHelper.create_test_person(db_session, name="Person1")
...
class TestComplexQueries:
    ...
    async def test_photos_with_faces_query(self, db_session: AsyncSession):
        ...
        # Create test data
        person = await DatabaseTestHelper.create_test_person(db_session)
        file_version_with_face = await DatabaseTestHelper.create_test_file_version(db_session)
        file_version_without_face = await DatabaseTestHelper.create_test_file_version(db_session)
    ...
    async def test_high_rated_photos_query(self, db_session: AsyncSession):
        ...
        high_rated = await DatabaseTestHelper.create_test_file_version(db_session, rating=5)
        medium_rated = await DatabaseTestHelper.create_test_file_version(db_session, rating=3)
        low_rated = await DatabaseTestHelper.create_test_file_version(db_session, rating=1)
    ...
    async def test_person_photo_count_query(self, db_session: AsyncSession):
        ...
        # Create test data
        person1 = await DatabaseTestHelper.create_test_person(db_session, name="Person 1")
        person2 = await DatabaseTestHelper.create_test_person(db_session, name="Person 2")
        ...
        await DatabaseTestHelper.create_test_face(db_session, file_version=photo1, person=person1)
        await DatabaseTestHelper.create_test_face(db_session, file_version=photo2, person=person1)
        await DatabaseTestHelper.create_test_face(db_session, file_version=photo3, person=person2)
...
class TestDatabaseConstraints:
    ...
    async def test_cascade_deletions(self, db_session: AsyncSession):
        ...
        asset = await DatabaseTestHelper.create_test_media_asset(db_session)
        version1 = await DatabaseTestHelper.create_test_file_version(db_session, media_asset=asset)
        version2 = await DatabaseTestHelper.create_test_file_version(db_session, media_asset=asset)
        ...
        assert len(remaining_versions) == 0 or len(remaining_versions) == 2  # Depends on configuration
