...
from tests.utils import (
    ...
    FileTestHelper, PerformanceTestHelper
)
...
class TestDatabasePerformance:
    ...
    async def test_photo_query_performance(self, db_session):
        ...
        for i in range(photo_count):
            photo = await DatabaseTestHelper.create_test_file_version(
                ...
                location=f"{40.7589 + i * 0.001},{-73.9851 + i * 0.001}" if i % 2 == 0 else None
            )
            photos.append(photo)
        ...
        query_tests = [
            ("Simple select all", lambda: db_session.query(FileVersion).limit(100).all()),
            ("Filter by rating", lambda: db_session.query(FileVersion).filter(FileVersion.rating >= 4).limit(100).all()),
            ("Filter by keywords", lambda: db_session.query(FileVersion).filter(FileVersion.keywords.contains(["tag_1"])).limit(100).all()),
            ("Filter with location", lambda: db_session.query(FileVersion).filter(FileVersion.location.isnot(None)).limit(100).all()),
            ("Order by rating desc", lambda: db_session.query(FileVersion).order_by(FileVersion.rating.desc()).limit(100).all()),
            ("Complex filter", lambda: db_session.query(FileVersion).filter(
                FileVersion.rating >= 3,
                FileVersion.location.isnot(None)
            ).order_by(FileVersion.created_at.desc()).limit(50).all())
        ]
        ...
        for test_name, query_func in query_tests:
            ...
            print(f"{test_name}: avg={avg_time:.3f}s, min={min_time:.3f}s, max={max_time:.3f}s")
            
            # Performance assertions
            assert avg_time < 1.0, f"{test_name} average time {avg_time:.3f}s exceeds 1.0s threshold"
            assert max_time < 2.0, f"{test_name} max time {max_time:.3f}s exceeds 2.0s threshold"
    ...
    async def test_face_detection_query_performance(self, db_session):
        ...
        face_query_tests = [
            ("Get all faces for person", lambda p: db_session.query(Face).filter(Face.person_id == p.id).all()),
            ("Get unassigned faces", lambda p: db_session.query(Face).filter(Face.person_id.is_(None)).limit(100).all()),
            ("Count faces per person", lambda p: db_session.query(Face).filter(Face.person_id == p.id).count()),
            ("Get people with face count", lambda p: db_session.query(Person).filter(Person.face_count > 10).all())
        ]
        ...
        for test_name, query_func in face_query_tests:
            ...
            assert query_time < 0.5, f"{test_name} time {query_time:.3f}s exceeds 0.5s threshold"
    ...
    async def test_collection_query_performance(self, db_session):
        ...
        collections_with_counts = await asyncio.to_thread(
            lambda: db_session.query(Collection).join(CollectionPhoto).group_by(Collection.id).all()
        )
        end = time.time()
        
        query_time = end - start
        print(f"Collections with photo counts: {query_time:.3f}s")
        assert query_time < 2.0, f"Collection query time {query_time:.3f}s exceeds 2.0s threshold"
...
class TestAPIPerformance:
    ...
    async def test_photo_listing_performance(self, async_client, db_session):
        ...
        for endpoint, test_name in test_cases:
            ...
            assert avg_time < 1.0, f"{test_name} average time {avg_time:.3f}s exceeds 1.0s"
    
    async def test_search_performance(self, async_client, db_session):
        """Test search API performance."""
        # Create searchable dataset
        search_terms = ["vacation", "work", "family", "food", "nature", "city", "beach", "mountain"]
        ...
        search_tests = [
            ...
            ("/api/search?q=vacation&page=1&per_page=20", "Text search with pagination"),
            ("/api/search/location?latitude=40.7589&longitude=-73.9851&radius_km=10", "Location search"),
        ]
        ...
        for endpoint, test_name in search_tests:
            ...
            assert search_time < 2.0, f"{test_name} time {search_time:.3f}s exceeds 2.0s"
...
class TestLoadTesting:
    ...
    async def test_concurrent_photo_requests(self, async_client, db_session):
        ...
        for concurrency in concurrency_levels:
            ...
            print(f"Concurrency {concurrency}: total={total_time:.3f}s, "
    ...
    async def test_concurrent_search_requests(self, async_client, db_session):
        ...
        print(f"Concurrent searches: {concurrent_searches} requests in {total_time:.3f}s, "
    ...
    async def test_upload_stress_test(self, async_client, temp_dir):
        ...
        with patch('app.services.ai_service.ai_service.analyze_image') as mock_ai, \
            ...
            mock_ai.return_value = {"tags": ["test"], "description": "Test image", "confidence": 90}
            ...
            start_time = time.time()
            upload_tasks = [upload_file(path) for path in image_paths[:10]]  # 10 concurrent uploads
            ...
            print(f"Concurrent uploads: 10 files in {total_time:.3f}s, "
                  f"avg={avg_upload_time:.3f}s, max={max_upload_time:.3f}s")
            
            assert avg_upload_time < 2.0, f"Average upload time {avg_upload_time:.3f}s too high"
...
class TestMemoryAndResourceUsage:
    ...
    async def test_memory_usage_during_bulk_operations(self, async_client, db_session):
        ...
        assert memory_growth < 500, f"Memory growth {memory_growth:.1f}MB exceeds 500MB threshold"
    ...
    async def test_database_connection_pooling(self, async_client, db_session):
        ...
        async def db_intensive_request() -> float:
            ...
            if photos:
                detail_response = await async_client.get(f"/api/photos/{photos[0]['id']}")
        ...
        print(f"DB connection test: {concurrent_requests} concurrent requests, "
...
class TestBenchmarks:
    ...
    async def test_image_processing_benchmark(self, temp_dir):
        ...
        with patch('app.services.thumbnail_service.thumbnail_service.generate_thumbnails') as mock_thumbnails, \
            ...
            for size, processing_time in benchmark_results.items():
                assert processing_time < 1.0, f"Processing time for {size} is {processing_time:.3f}s (>1.0s)"
    ...
    async def test_search_indexing_benchmark(self, db_session):
        ...
        for keyword_count, keywords in keyword_scenarios:
            ...
            for photo in photos[:10]:  # Test subset
                ...
                await asyncio.to_thread(
                    lambda: db_session.query(FileVersion).filter(
                        FileVersion.keywords.contains(keywords[:1])
                    ).all()
                )
            ...
            print(f"Search with {keyword_count} keywords: {avg_time_per_search:.3f}s per search")
        ...
        for keyword_count, search_time in indexing_times.items():
            assert search_time < 0.1, f"Search time with {keyword_count} keywords is {search_time:.3f}s (>0.1s)"
...
class TestPerformanceRegression:
    ...
    async def test_api_response_time_regression(self, async_client, db_session):
        ...
        for endpoint, baseline_ms in performance_baselines.items():
            ...
            print(f"{endpoint}: avg={avg_time:.3f}s, p95={p95_time:.3f}s, baseline={baseline_ms}s")
            
            # Check against baseline (allowing 20% margin)
            assert avg_time < baseline_ms * 1.2, \
                f"{endpoint} average time {avg_time:.3f}s exceeds baseline {baseline_ms}s by >20%"
            assert p95_time < baseline_ms * 1.5, \
    ...
    async def test_database_query_performance_regression(self, db_session):
        ...
        query_baselines = {
            ...
            "filtered_select": (
                lambda: db_session.query(FileVersion).filter(FileVersion.rating >= 4).limit(100).all(),
                0.2  # 200ms
            ),
            "ordered_select": (
                lambda: db_session.query(FileVersion).order_by(FileVersion.rating.desc()).limit(100).all(),
                0.3  # 300ms
            ),
            "count_query": (
        ...
        for query_name, (query_func, baseline_s) in query_baselines.items():
            ...
            print(f"{query_name}: avg={avg_time:.3f}s, max={max_time:.3f}s, baseline={baseline_s}s")
            
            # Check against baseline
            assert avg_time < baseline_s * 1.5, \
                f"{query_name} average time {avg_time:.3f}s exceeds baseline {baseline_s}s by >50%"
            assert max_time < baseline_s * 2.0, \
