...
class TestCRUDOperations:
    ...
    async def test_person_crud(self, db_session: AsyncSession):
        ...
        # Search by name
        search_results = await person.search_by_name(db_session, name_query="Jane")
...
class TestRelationships:
    ...
    async def test_media_asset_file_versions_relationship(self, db_session: AsyncSession):
        ...
        for tier in ["bronze", "silver", "gold"]:
            version = FileVersion(
                media_asset_id=asset.id,
                tier=tier,
                file_path=f"/path/to/{tier}.jpg",
                file_hash=f"hash_{tier}",
                file_size=1024 * (1 if tier == "bronze" else 2 if tier == "silver" else 4),
                mime_type="image/jpeg"
            )
            db_session.add(version)
        ...
        asset_versions = await file_version.get_by_media_asset(db_session, asset.id)
    ...
    async def test_person_face_relationship(self, db_session: AsyncSession):
        ...
        for i in range(3):
            face_obj = Face(
                photo_id=version.id,
                person_id=person_obj.id,
                bounding_box={"top": 10+i*10, "left": 20+i*10, "bottom": 100+i*10, "right": 120+i*10},
                confidence=90 + i
            )
            db_session.add(face_obj)
        ...
        # Test relationship
        person_faces = await face.get_by_person(db_session, person_obj.id)
        assert len(person_faces) == 3
        
        photo_faces = await face.get_by_photo(db_session, version.id)
